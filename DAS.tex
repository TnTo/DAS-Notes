...
\section{Model steps}
\begin{steps}
	\item[C1] \label{C1} Every Q times, Central Bank updates interest rates []
	\item[G1] \label{G1} Every T times, Government update tax policies []
	\item[I1] \label{I1} Financial Intermediaries set target interest rate [\cref{C1}]
	\item[G2] \label{G2} Government set target output for Public Sector Consumption Firms []
	\item[F1] \label{F1} Consumption Firms set desired output and order Capital goods to Capital Firms [\cref{G2}]
	\item[F2] \label{F2} Capital Firms set desired output [\cref{C1,F1}]
	\item[F3-I2] \label{F3-I2} Financial Intermediaries put sell and buy order on Equities' market, Firms emit new equities [\cref{I1,F1,F2}]
	\item[F4] \label{F4} Firms acquire Loans from Banks [\cref{F3-I2}]
	\item[F5] \label{F5} Firms open job vacancies [\cref{F3}]
	\item[H1] \label{H1} Households chose if enter or exit job market []
	\item[F6] \label{F6} Firms hire [\cref{F4,H1}]
	\item[H2] \label{H2} Households set demand for Consumption Goods and Financial Intermediaries' Shares [\cref{F5,I1}]
	\item[F7] \label{F7} Wages are paid [\cref{F6}]
