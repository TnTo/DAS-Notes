...
\section{Model steps}
\begin{steps}
	\item \label{C1} Every Q times, Central Bank updates interest rates [$\mathcal{C}$]
	\item \label{H1} Households set demand for Consumption Goods [\cref{C1}, $\mathcal{H}$]
	\item \label{G1} Government set target output for Public Sector Consumption Firms [$\mathcal{G}$]
