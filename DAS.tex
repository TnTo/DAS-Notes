...
\begin{steps}
	...
	\item \label{I2} Financial Intermediaries put sell and buy order on Equities' market, Firms emit new equities [\cref{I1,F1,F2}]
	\item \label{F3} Firms acquire Loans from Banks [\cref{I2}]
	\item \label{F4} Firms open job vacancies [\cref{F3}]
	\item \label{H1} Households chose if enter or exit job market []
	\item \label{F5} Firms hire [\cref{F4,H1}]
	\item \label{H2} Households set demand for Consumption Goods and Financial Intermediaries' Shares [\cref{F5,I1}]
	\item \label{F6} Wages are paid [\cref{F5}]
