...
\begin{enumerate}
	...
	\litem{I1} Financial Intermediaries set target interest rate [\ref{C1}]
	\litem{G2} Government set target output for Public Sector Consumption Firms []
	\litem{F1} Consumption Firms set desired output and order Capital goods to Capital Firms [\ref{G2}]
	\litem{F2} Capital Firms set desired output [\ref{C1,F1}]
	\litem{F3-I2} Financial Intermediaries put sell and buy order on Equities' market, Firms emit new equities [\ref{I1,F1,F2}]
	\litem{F4} Firms acquire Loans from Banks [\ref{F3-I2}]
	\litem{F5} Firms open job vacancies [\ref{F3}]
	\litem{H1} Households chose if enter or exit job market []
	\litem{F6} Firms hire [\ref{F4,H1}]
	\litem{H2} Households set demand for Consumption Goods and Financial Intermediaries' Shares [\ref{F5,I1}]
	\litem{F6} Wages are paid [\ref{F5}]
